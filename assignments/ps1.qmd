...
Your turn. Define a new variable `budget_mil` which measures budget in MILLIONS of dollars (on the 2013 scale). Similarly, define a new variable `intgross_mil` which measures international gross in MILLIONS of dollars (on the 2013 scale). Store these in `bechdel` *and* show the first 3 rows of `budget_mil` and `intgross_mil`:
...
Before moving on, check that your results match those below (within rounding).
If they do not, your results to the remaining exercises will also be incorrect. 
...
*Reminder: Render your qmd often!*

Complete each task below using `dim()`, `nrow()`, `head()`, or one of the `dplyr` functions: `filter()`, `summarize()`, `select()`. You don't need to write out any discussion for this exercise, just include your code and output.

**IMPORTANT IMPORTANT IMPORTANT:** For any questions about movie budget and gross earnings, use `budget_mil` and `gross_mil` (these are already adjusted to 2013 dollars).
...
# What is the median film budget?

# What films had budgets that exceeded 300 million?
...
Construct a visualization of the `clean_test` variable, and construct a corresponding table of counts. In one sentence, summarize your findings about `clean_test`.
...
a. Construct a visualization of the `intgross_mil` variable. Summarize your findings in words.

b. Calculate the mean and median values of `intgross_mil`. Explain why the mean exceeds the median by quite a bit, *and* explain which is the more appropriate measure of trend here.
...
a. In the relationship between `intgross_mil` and `budget_mil`, which is the *response* variable and which is the *predictor* variable?
...
c. In 1-3 sentences, summarize your findings about this relationship. Be sure to comment on the *trend* and *variability* in this relationship, as well as on any *outliers* (and in what way these movies are outliers).
...
b. Write out a formula for the trend. Pay close attention to details on both the left hand and right hand side of the formula.

c. Interpret the `budget_mil` coefficient in context. *Make sure to use non-causal language, include units, and talk about averages rather than individual cases.*
...
**NOTE:** This section is required even if you didn't use any of these resources.
