...
- Determining when an F-test (or more generally a test for multiple coefficients) is needed/appropriate for a specific hypothesis
...
Let's consider "value" in terms of statistical significance, i.e. by using our data to test appropriate hypotheses.
To this end, just as with multiple linear regression, we can test hypotheses about 1 or more model coefficient in our logistic regression model!
Though the concepts are the same, these tests have different names in logistic regression:

- a test for 1 coefficient: in linear regression this is a "t-test", but in logistic regression it's a "z-test" 
- a test for multiple coefficients (by comparing nested models): in linear regression this is an "F-test", but in logistic regression it's a "Chi-squared test" 
...
THINK: Can you use a z-test (for one coefficient) from the original model summary table, or do you need to do a Chi-squared test (for multiple coefficients)?
